...
export default function ExcelUploader() {
  ...
  const checkFileWithN8N = async (file) => {
    ...
    try {
      const res = await fetch(
        "https://ai.bmspcustomer.net/webhook/excelCheck",
        {
          method: "POST",
          body: formData,
          headers: {
  ...
  const sendFileToN8N = async () => {
    ...
    try {
      ...
      if (data.editedFileUrl || data.downloadUrl || data.fileUrl) {
        setEditedFileUrl(
          data.editedFileUrl || data.downloadUrl || data.fileUrl
        );
      }
  ...
  const exportCleanedData = () => {
    ...
    const cleanedData = excelData.filter(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br mb-3">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-2 tracking-wide mt-3">
            Excel File Uploader
          </h1>
